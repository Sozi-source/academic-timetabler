...
import {
  stageUnitImportAction,
} from './actions';
import {
  initialUnitImportActionState,
} from './types';

export function UnitImportUploadForm() {
  ...
  useEffect(() => {
    if (
      ...
      router.push(
        `/timetable/units/import/${state.batchId}`,
      );
    }
  }, [
    router,
      ...
      <section className="space-y-4 rounded-2xl border border-border bg-surface p-5">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-text-muted">
            The importer validates Programme Codes,
            Academic Period numbers, contact hours,
            room preferences and duplicate units.
          </p>
        </div>

        <label
          htmlFor="unit-import-workbook"
