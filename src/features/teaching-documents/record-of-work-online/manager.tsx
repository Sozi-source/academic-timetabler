...
function RecordLessonModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-xs animate-in fade-in">
      <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-2xl overflow-hidden animate-in zoom-in-95">
        {/* Modal Header */}
        ...
        <form action={action} className="p-5 space-y-4">
          ...
          {/* Scheme Sync Notification */}
          {(occurrence.topicSuggestion || occurrence.objectivesSuggestion) ? (
            <div className="flex items-center gap-2 rounded-lg bg-teal-50 px-3 py-2 text-xs font-semibold text-teal-900 border border-teal-200">
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              rows={2}
              required
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 outline-none focus:border-teal-700 focus:ring-1 focus:ring-teal-700 leading-relaxed"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <input
                name="deliveryMode"
                defaultValue={occurrence.deliveryMode || 'Lecture & Discussion'}
                required
                className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 outline-none focus:border-teal-700"
              />
            </div>
            <div className="space-y-1">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <input
                ...
                className="rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-900 outline-none focus:border-teal-700"
          ...
          <div className="flex items-center justify-end gap-2 border-t border-slate-200 pt-3">
            ...
            <button
              type="submit"
              disabled={pending}
              className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg bg-teal-900 px-4 text-xs font-bold text-white shadow-xs hover:bg-teal-800 disabled:opacity-60"
            >
              <CheckCircle2 className="size-4" />
              {pending ? 'Saving...' : 'Confirm & Submit Entry'}
            </button>
          </div>
        </form>
      </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <Link
            ...
            className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
          >
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1 border-t border-slate-100 pt-3 text-xs text-slate-600">
          ...
          <span className="ml-auto text-teal-800 font-bold">
            {submitted.length} recorded · {pending.length} pending
          </span>
        </div>
      </section>

      {/* 2. UNIFIED SIMPLE RECORD OF WORK TABLE */}
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xs">
        {!context.timetable.available ? (
          <div className="p-8 text-center text-xs text-slate-500">
            <FileCheck2 className="mx-auto size-5 text-slate-400 mb-1" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left text-xs min-w-[720px]">
              ...
              <tbody className="divide-y divide-slate-200">
                {/* 1. Show Submitted Entries first */}
