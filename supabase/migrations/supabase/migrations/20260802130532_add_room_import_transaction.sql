-- ============================================================
-- HND App: Transactional Room Bulk Import
-- ============================================================

create or replace function
  public.import_valid_room_rows(
...
begin
  if not public.current_user_has_role(
    ...
    raise exception using
      errcode = '42501',
      message =
        'You are not authorized to import rooms';
  end if;
  ...
  if selected_batch.entity_type <> 'rooms' then
    raise exception using
      errcode = 'P0001',
      message =
        'The selected import batch is not a Rooms import';
  end if;
  ...
  if selected_batch.status <> 'validated' then
    raise exception using
      errcode = 'P0001',
      message =
        'Only validated Rooms import batches can be confirmed';
  end if;
...
revoke all
on function
  public.import_valid_room_rows(uuid)
from public;
...
comment on function
  public.import_valid_room_rows(uuid)
is
  'Atomically imports all valid staged Room rows and completes the import audit batch.';