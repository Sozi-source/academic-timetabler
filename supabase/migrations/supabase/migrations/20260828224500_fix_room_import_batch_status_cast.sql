-- PostgreSQL resolves the CASE branches as text. Cast the result back to the
-- import_batch_status enum used by import_batches.status.
...
comment on function public.import_valid_room_rows(uuid) is
  'Atomically imports valid staged Room rows, maps floor to floor_label, and preserves enum-safe batch status updates.';
