-- Preserve distinct January/March admission cohorts while sharing one operational progression group.
-- This replaces only the student import RPC; no existing student or cohort records are rewritten.
  ...
  loop
    ...
    begin
      ...
      if normalized->>'currentCohortId'=normalized->>'admissionCohortId' then
        ...
        -- January and March are distinct admission cohorts but share one
        -- operational progression group from the point of admission. Preserve
        -- the historical admission cohort on students.admission_cohort_id and
        -- keep only the operational study cohort open in assignment history.
        insert into public.student_cohort_assignments(
          student_id,cohort_id,effective_from,is_admission_cohort,assignment_reason,notes
        ) values (
