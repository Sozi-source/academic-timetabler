-- Migration: 20260920172000_fix_daily_report_attendance_multi_key.sql
-- Description:
--   Upgrades get_trainer_daily_report_workspace and submit_trainer_daily_report_v1
--   to match class_sessions by BOTH scheduled_session_id AND teaching_allocation_id + date.
--
--   Root cause: After migration 20260920150000 decoupled class_sessions.scheduled_session_id
--   (ON DELETE SET NULL), existing cancelled ("Did Not Take Place") records where the FK drifted
--   to NULL are no longer found by the lateral join that only matched on scheduled_session_id.
--   This made all those sessions re-appear as overdue/unrecorded for every trainer.
...
begin
  ...
  -- Build draft workspace with multi-key attendance lookup
  with schedule as (
    select *
  ...
  enriched as (
    ...
    from schedule
    left join lateral (
      -- Multi-key: prefer exact scheduled_session_id match;
      -- fall back to teaching_allocation_id where scheduled_session_id drifted to NULL
      ...
      where session.session_date = target_report_date
        and (
          ...
          or (
            session.scheduled_session_id is null
            and session.teaching_allocation_id = schedule.teaching_allocation_id
          )
        )
      order by
        (case when session.scheduled_session_id = schedule.scheduled_session_id then 0 else 1 end),
...
comment on function public.get_trainer_daily_report_workspace(date) is
  'Builds trainer daily report workspace with multi-key attendance matching (scheduled_session_id OR teaching_allocation_id+date) so cancelled sessions are always recognised even after timetable regeneration.';
      ...
      where not exists (
        ...
        where attendance.session_date = target_report_date
          ...
          and (
            ...
            or (
              attendance.scheduled_session_id is null
...
comment on function public.submit_trainer_daily_report_v1(date, text, text) is
  'Submits the daily report after multi-key completeness check (scheduled_session_id OR teaching_allocation_id+date) so cancelled sessions are always recognised.';
