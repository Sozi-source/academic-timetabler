...
interface ImportBatchRow {
  ...
  status:
    TeachingAllocationImportBatch['status'];
  total_rows: number;
  valid_rows: number;
  invalid_rows: number;
...
function mapStagedRow(
  ...
  } as TeachingAllocationImportStagedRow;
}

export const getTeachingAllocationImportBatch =
  cache(
    async (
      batchId: string,
    ): Promise<{
      ...
      if (batchError) {
        throw new Error(
          `Unable to load the Teaching Allocation import batch: ${batchError.message}`,
        );
      }
      ...
      if (rowsError) {
        throw new Error(
          `Unable to load staged Teaching Allocation rows: ${rowsError.message}`,
        );
      }
