...
import {
  confirmTrainerImportAction,
} from './actions';
import {
  initialTrainerImportActionState,
} from './types';

interface TrainerImportConfirmationProps {
  batchId: string;
  validRows: number;
  invalidRows: number;
...
export function TrainerImportConfirmation({
  batchId,
  validRows,
  invalidRows,
  duplicateRows,
  batchStatus,
}: TrainerImportConfirmationProps) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-semibold text-text-primary">
              Trainer import completed
            </h2>

            <p className="mt-1 text-sm leading-6 text-text-secondary">
              The validated Trainer records have been
              processed and the audit batch is complete.
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-border bg-surface p-5">
      <div>
        <h2 className="font-semibold text-text-primary">
          Confirm Trainer import
        </h2>

        <p className="mt-1 text-sm leading-6 text-text-muted">
          Only rows marked Ready will be inserted.
          Invalid and duplicate rows will be retained
          in the audit record and marked as skipped.
        </p>
      </div>
