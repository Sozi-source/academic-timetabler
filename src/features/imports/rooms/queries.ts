...
export const getRoomImportBatch = cache(
  async (
    batchId: string,
  ): Promise<{
    ...
    if (batchError) {
      throw new Error(
        `Unable to load the Rooms import batch: ${batchError.message}`,
      );
    }
    ...
    if (rowsError) {
      throw new Error(
        `Unable to load staged Rooms rows: ${rowsError.message}`,
      );
    }
