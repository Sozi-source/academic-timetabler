-- Migration: Scope trainer propagation in move_scheduled_session_safely
--
-- Problem fixed:
-- 1. The previous version unconditionally updated the parent teaching
--    allocation's trainer AND every active sibling session's trainer on
--    every call, even a pure day/time/room move where the trainer never
--    changed. This gave a single-session "minor" edit a blast radius of
--    every session for that allocation, and re-stamped updated_at/updated_by
--    on rows nothing actually changed about.
-- 2. Because target_trainer_id defaults to null, a caller that omits it
--    (rather than passing the session's existing trainer) would silently
--    wipe the trainer from the allocation and every sibling session.
--
-- Fix: only touch the allocation + sibling sessions when target_trainer_id
-- is provided AND differs from the session's current trainer. A plain move
-- (day/slot/room/notes only) now updates exactly one scheduled_sessions row.
...
begin
  ...
  -- Only a genuine trainer change should propagate anywhere beyond this row.
...
comment on function public.move_scheduled_session_safely(uuid,uuid,uuid,uuid,uuid,text,uuid) is
  'Safely updates a single timetable session day, slots, room, notes, and (only when actually changing) trainer, with complete clash detection. Sibling sessions and the parent allocation are only touched when the trainer changes.';
