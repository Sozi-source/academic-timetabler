...
import {
  confirmRoomImportAction,
} from './actions';
import {
  initialRoomImportActionState,
} from './types';

interface RoomImportConfirmationProps {
  batchId: string;
  validRows: number;
  invalidRows: number;
...
export function RoomImportConfirmation({
  batchId,
  validRows,
  invalidRows,
  duplicateRows,
  batchStatus,
}: RoomImportConfirmationProps) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-text-secondary">
              Valid Room records have been inserted
              and the import audit is complete.
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-border bg-surface p-5">
      <div>
        <h2 className="font-semibold text-text-primary">
          Confirm Rooms import
        </h2>

        <p className="mt-1 text-sm leading-6 text-text-muted">
          Only rows marked Ready will be inserted.
          Invalid and duplicate records will be
          retained in the audit history and skipped.
        </p>
      </div>
