...
-- ============================================================
-- The department/HOD registration RPC must be SECURITY DEFINER
-- because authenticated users intentionally do not have broad
-- direct write privileges on student_unit_registrations.
--
-- Safety remains enforced inside the function via:
-- - current_user_can_manage_department(...)
-- - active academic period validation
-- - student eligibility validation
-- - programme/unit offering validation
-- - exception note requirement when expected units are changed
...
revoke all on function public.department_register_student_units(uuid, uuid, uuid[], text)
  from public, anon;

grant execute on function public.department_register_student_units(uuid, uuid, uuid[], text)
  to authenticated;

comment on function public.department_register_student_units(uuid, uuid, uuid[], text) is
  'Secure HOD-side unit registration. SECURITY DEFINER is used to avoid broad table write grants while retaining explicit department authorization and validation.';
