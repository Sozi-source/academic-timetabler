...
import {
  stageTrainerImportAction,
} from './actions';
import {
  initialTrainerImportActionState,
} from './types';

export function TrainerImportUploadForm() {
  ...
  useEffect(() => {
    if (
      ...
      router.push(
        `/timetable/trainers/import/${state.batchId}`,
      );
    }
  }, [
    router,
      ...
      <section className="rounded-2xl border border-border bg-surface p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            ...
            <TemplateDownloadLink
              entityType="trainers"
              label="Download Trainers template"
              className="mt-2"
            />
          </div>
        </div>
      ...
      <section className="space-y-4 rounded-2xl border border-border bg-surface p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">
            Maximum file size: 50 MB. Only .xlsx
            files generated from the current template
            are accepted.
          </p>
        </div>

        <label
          htmlFor="trainer-import-workbook"
