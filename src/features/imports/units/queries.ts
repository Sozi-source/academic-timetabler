...
export const getUnitImportBatch = cache(
  async (
    batchId: string,
  ): Promise<{
    batch: UnitImportBatch;
    ...
    if (batchError) {
      throw new Error(
        `Unable to load the Units import batch: ${batchError.message}`,
      );
    }
    ...
    if (rowsError) {
      throw new Error(
        `Unable to load staged Units rows: ${rowsError.message}`,
      );
    }
