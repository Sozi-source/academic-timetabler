...
export default async function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[1fr_0.7fr]">
      <Card><CardHeader><p className="text-xs font-semibold uppercase tracking-[0.16em] text-text-muted">First time setup</p><h2 className="mt-1.5 text-sm font-semibold text-text-primary">Add only the information the timetable needs</h2><p className="mt-1 text-xs text-text-secondary">You can return to these settings at any time.</p></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2">{setupLinks.map(([label, href], index) => <Link key={href} href={href} className="flex h-10 items-center gap-2.5 rounded-xl border border-border-soft px-3.5 text-sm font-medium text-text-secondary transition hover:border-primary hover:bg-primary-soft hover:text-primary"><CheckCircle2 className="size-4 shrink-0 text-primary" />{index + 1}. {label}</Link>)}</CardContent></Card>
      <Card><CardHeader><div className="flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-xl bg-primary-soft text-primary"><CalendarDays className="size-4" /></span><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-text-muted">Recommended</p><h2 className="mt-0.5 text-sm font-semibold text-text-primary">Check before generating</h2></div></div></CardHeader><CardContent><p className="text-xs leading-5 text-text-secondary">The setup check tells you exactly what is missing. Fix only the highlighted items, then generate the timetable.</p><Link href="/timetable/readiness" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-primary">Open setup check <ArrowRight className="size-3.5" /></Link></CardContent></Card>
    </section>
  </div>;
}
