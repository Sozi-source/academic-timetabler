...
import {
  confirmTeachingAllocationImportAction,
} from './actions';
import {
  initialTeachingAllocationImportActionState,
} from './types';

interface TeachingAllocationImportConfirmationProps {
  batchId: string;
  validRows: number;
  invalidRows: number;
...
export function TeachingAllocationImportConfirmation({
  batchId,
  validRows,
  invalidRows,
  duplicateRows,
  batchStatus,
}: TeachingAllocationImportConfirmationProps) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-semibold text-text-primary">
              Teaching Allocations import completed
            </h2>

            <p className="mt-1 text-sm leading-6 text-text-secondary">
              Valid allocations have been registered
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-border bg-surface p-5">
      <div>
        <h2 className="font-semibold text-text-primary">
          Confirm allocation import
        </h2>

        <p className="mt-1 text-sm leading-6 text-text-muted">
          Only rows marked Ready will be inserted.
          The database will recheck trainer workload
          and scheduling constraints transactionally.
        </p>
      </div>
      ...
      {!canImport ? (
        <p className="text-right text-xs text-text-muted">
          There are no valid staged allocations
