-- PostgreSQL resolves the branches of an untyped CASE expression as text.
-- Recreate the Trainer importer with explicit import_batch_status casts so
-- confirmation works on databases where the previous migration is applied.
...
begin
  ...
  set status = 'skipped'::public.import_row_status
  where import_batch_id = target_batch_id
    and status in (
      'invalid'::public.import_row_status,
      'duplicate'::public.import_row_status
    );
  ...
  for staged_row in
    ...
    where import_batch_id = target_batch_id
      and status = 'valid'::public.import_row_status
  ...
  loop
    ...
    update public.import_rows
    set
      status = 'imported'::public.import_row_status,
...
comment on function public.import_valid_trainer_rows(uuid) is
  'Imports Trainer rows into their home workspace with explicit enum-safe batch and row status transitions.';
