-- One-click conversion of every currently unassigned unit in the working
-- department into a trainer-pending allocation that the generator can place.
...
begin
  ...
  -- Ensure each readiness participant has a corresponding Unit on Offer. The
  -- insert is department-scoped and leaves existing unit-offering settings
  -- unchanged.
  ...
  -- Preserve already-defined shared teaching offerings by linking every
  -- participant Unit on Offer to the same confirmed schedulable class before
  -- trainer-pending allocations are created.
  ...
  -- Reconcile Units on Offer with the allocation register. This keeps the
  -- bulk action idempotent even when an allocation was created before the
  -- corresponding Unit on Offer record was generated.
  update public.unit_offerings offering
  set
...
comment on function public.include_all_unassigned_unit_offerings(uuid) is
  'Creates trainer-pending allocations for all unassigned Units on Offer in the current working department.';
